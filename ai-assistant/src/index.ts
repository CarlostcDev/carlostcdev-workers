...
interface RequestBody {message?: unknown;messages?: unknown;}
const corsHeaders = {
...
const streamHeaders = {...corsHeaders, "Content-Type": "text/event-stream; charset=utf-8", "Cache-Control": "no-cache"} satisfies Record<string, string>;

export default {
	async fetch(request: Request, env: Env): Promise<Response> {
		if (request.method === "OPTIONS") return new Response(null, { status: 204, headers: corsHeaders });
		if (request.method !== "POST") return new Response("Method Not Allowed", { status: 405, headers: corsHeaders });
		if (!env.GEMINI_API_KEY) return jsonResponse({ error: "Falta GEMINI_API_KEY" }, 500);
		const body = await readBody(request);
		if (!body) return jsonResponse({ error: "JSON inválido" }, 400);
		const contents = normalizeContents(body);
		if (contents.length === 0) return jsonResponse({ error: "El historial de mensajes es inválido" }, 400);
		try {
			...
			if (!geminiResponse.ok) return jsonResponse({ error: "Error de Gemini", details: await geminiError(geminiResponse) }, geminiResponse.status);
			return new Response(geminiResponse.body, {status: 200, headers: streamHeaders});
		} catch {
...
async function readBody(request: Request): Promise<RequestBody | null> {
	try {return await request.json() as RequestBody;}
	catch {return null;}
}
...
function normalizeContents(body: RequestBody): GeminiContent[] {
	...
	if (typeof body.message === "string" && body.message.trim().length > 0) return [{ role: "user", parts: [{ text: body.message.trim() }] }];
	return [];
}
...
function normalizePart(value: unknown): GeminiPart[] {
	if (isRecord(value) && typeof value.text === "string" && value.text.trim().length > 0) return [{ text: value.text.trim() }];
	return [];
}

function normalizeContent(value: unknown): GeminiPart[] {
	if (typeof value === "string" && value.trim().length > 0) return [{ text: value.trim() }];
	if (Array.isArray(value)) {
		return value.flatMap((item) => {
			if (typeof item === "string") return normalizeContent(item);
